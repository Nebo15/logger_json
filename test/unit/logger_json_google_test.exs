defmodule LoggerJSONGoogleTest do
  ...
  describe "google_error_reporter" do
    setup do
      Logger.configure_backend(LoggerJSON, metadata: :all)
      :ok = Application.put_env(:logger_json, :google_error_reporter, %{})
      on_exit(fn -> :ok = Application.delete_env(:logger_json, :google_error_reporter) end)
    end

    test "formats elixir error for google" do
      stacktrace = [
        {Foo, :bar, 0, [file: 'foo/bar.ex', line: 123]},
        {Foo.Bar, :baz, 1, [file: 'foo/bar/baz.ex', line: 456]}
      ]
      Logger.metadata(crash_reason: {%RuntimeError{message: "oops"}, stacktrace})
    ...
    test "formats erlang error for google" do
      stacktrace = [
        {Foo, :bar, [], []},
        {Foo, :bar, 0, [file: 'foo/bar.ex', line: 123]},
        {Foo.Bar, :baz, 1, [file: 'foo/bar/baz.ex', line: 456]}
      ]
      Logger.metadata(crash_reason: {:undef, stacktrace})
    ...
    test "adds google_error_reporter metadata" do
      :ok = Application.put_env(:logger_json, :google_error_reporter, %{serviceContext: %{service: "myapp", version: "abc123"}})
      Logger.metadata(crash_reason: {%RuntimeError{message: "oops"}, []})
